...
class MultitaskPretrainingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Y1 = torch.Tensor(view/255. >= 0.25).squeeze().long()
        # initialize segmentation masks channels
        Y2 = np.zeros((self.view_size, self.view_size, len(ORDER)))
        ...
        Y2 = torch.Tensor(np.argmax(Y2, axis=(2))).long()
        # alignment and rotation tasks targets
        d = rotation % 90
        Y3 = int(min(d, 90 - d) <= self.alignment_threshold) + 1
        Y4 = torch.Tensor([-(360. - rotation)/180. if rotation > 180 else float(rotation)/180.]).float()
